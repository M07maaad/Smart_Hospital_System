    ...
    <aside className={`w-64 ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900 border-l border-slate-200'} hidden md:flex flex-col shadow-xl z-20 transition-colors duration-300`}>
      ...
      <nav className={`flex-1 p-4 space-y-2 ${darkMode ? 'bg-slate-900' : 'bg-white'}`}>
        ...
        <div className={`mt-4 pt-4 border-t ${darkMode ? 'border-slate-800' : 'border-slate-200'} px-4`}>
          <p className={`text-xs font-bold mb-2 uppercase ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>Nurse Station</p>
          <button className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg text-sm transition-all ${darkMode ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-100'}`}>
